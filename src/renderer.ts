...
nomnoml.render = function (graphics: Graphics, config: Config, compartment: Compartment, setFont: SetFont){
	...
	function renderNode(node: Classifier, level: number){
		...
		node.compartments.forEach(function (part: Compartment, i: number){
			var s = i > 0 ? new Style({ stroke: style.stroke }) : style; // only style node title
			if (s.empty) return
			g.save()
	...
	snapToPixels()
	renderCompartment(compartment, new Style({ stroke: undefined }), 0)
	g.restore()
}
