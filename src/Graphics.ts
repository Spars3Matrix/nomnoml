...
interface Graphics {
  ...
  clear(): void
  circle(x: number, y: number, r: number): Chainable
  ellipse(center: number, w: number, h: number, start: number, stop: number): Chainable
  ...
  circuit(path: Vector[], offset?: number, s?: number): Chainable
