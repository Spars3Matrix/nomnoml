var skanaar: any = skanaar || {}

skanaar.plucker = function<T> (pluckerDef: any): (e:any) => any {
...
skanaar.max = function (list: any[], plucker: any): number {
...
skanaar.sum = function<T> (list: T[], plucker: string){
    var transform = skanaar.plucker(plucker)
...
skanaar.flatten = function<T> (lists: T[][]): T[]{
    var out: T[] = []
...
skanaar.find = function<T> (list: T[], predicate: (e:T)=>boolean){
    for(var i=0; i<list.length; i++)
        if (predicate(list[i]))
            return list[i]
    return undefined
...
skanaar.last = function<T> (list: T[]){
    return list[list.length-1]
}

skanaar.hasSubstring = function hasSubstring(haystack: string, needle: string){
    if (needle === '') return true
    if (!haystack) return false
    return haystack.indexOf(needle) !== -1
}

skanaar.format = function format(template: string /* variadic params */){
...
skanaar.merged = function (a: any, b: any) {
...
skanaar.indexBy = function<T>(list: T[], key: keyof T): { [key:string]: T } {
...
skanaar.uniqueBy = function<T> (list: T[], pluckerDef: string): T[] {
